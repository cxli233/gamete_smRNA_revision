...
left <- table_1_df %>% 
  ...
  geom_bar(stat = "identity", fill = "steelblue4", color = NA, alpha = 0.8) +
  geom_text(aes(label = count_text, y = 100), size = 5, fontface = "bold", hjust = 0, vjust = 0.5) +
...
mid <- table_1_df %>% 
  ...
  geom_text(aes(label = pro_text), size = 5, fontface = "bold") +
...
right <- table_1_df %>% 
  ...
  geom_text(aes(label = enrich_text), size = 5, fontface = "bold") +
  scale_x_discrete(labels = NULL) +
  scale_y_discrete(labels = NULL) +
  scale_fill_gradient(low = "white", high = "steelblue4") +
  labs(x = NULL,
